...
int Calc_blk(double** u, double** f, int N, double eps, double& time);  // блочная последовательная
...
int main(int argc, char **argv)
{
	...
	icnt = Calc_blk(u_blk, f, N, eps, stime);  // Вызов блочной функции расчета
	cout << "Solution time = " << stime << endl;
	cout << "Iterations =    " << icnt << endl;
	cout << "Results:\n";
	Output(u_blk, N);
	...
	if (CompareResults(u_ser, u_blk, N))
		cout << "Block serial matches serial!\n";
	else
		cout << "Block serial DOES NOT match serial!\n";
...
int Calc_blk(double** u, double** f, int N, double eps, double& time)
{
	double max;
	double h = 1.0 / (N + 1);
	int icnt = 0;
